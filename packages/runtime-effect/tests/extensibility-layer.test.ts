...
const createConfig = (): LoadedAppConfig => ({
	provider: { id: "anthropic", label: "Anthropic", models: [] } as unknown as LoadedAppConfig["provider"],
	modelId: "anthropic/model",
	model: { id: "anthropic/model", label: "model", provider: "anthropic", contextWindow: 200000 } as LoadedAppConfig["model"],
	thinking: "off",
	theme: "marvin",
	systemPrompt: "system",
...
describe("ExtensibilityLayer instrumentation", () => {
	it("records summaries including custom command count and tool details", async () => {
		...
		const baseProviders = Layer.mergeAll(
			...
			Layer.succeed(SessionManagerTag, { sessionManager: {} as any }),
		...
		const layer = Layer.provideMerge(
			ExtensibilityLayer({
				sendRef: { current: () => {} },
				builtinTools: [] as AgentTool<any, any>[],
				hasUI: false,
				loader,
			}),
			baseProviders,
