		...
		() => {
			...
			it("should handle text + tool call in same response and preserve thoughtSignature on subsequent requests", async () => {
				...
				const context: Context = {
					systemPrompt:
						"You are a helpful assistant. Think through your actions before using tools.",
					messages: [],
					tools: [readTool],
				};
				...
				if (!hasText || !hasToolCall) {
					console.log(
						"Model did not generate text + tool call in same response, skipping test",
					);
					return;
				}
				...
				// Provide tool result
				const toolCallBlock = firstResponse.content.find(
					(b) => b.type === "toolCall",
				);
